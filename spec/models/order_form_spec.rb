...
RSpec.describe OrderForm, type: :model do
  before do
    ...
    @order_form.product_id = FactoryBot.create(:user)
  end
  ...
  describe '配送先情報の保存' do
    ...
    context '配送先情報の保存ができないとき' do
      ...
      it '都道府県が「---」だと保存できないこと' do
        @order_form.area_ken_id = 0
        @order_form.valid?
        expect(@order_form.errors.full_messages).to include("Area ken can't be blank")
      end
      it '都道府県が空だと保存できないこと' do
        @order_form.area_ken_id = nil
        @order_form.valid?
        expect(@order_form.errors.full_messages).to include("Area ken can't be blank")
      end
